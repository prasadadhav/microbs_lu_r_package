test_that("getting the NULL path for created data as it is not set", {
...
test_that("getting the default path for created data, as it is set by default", {
    true_path <- "L:/Units & Programmes/BIOTECH/ENVMICRO/_Common/Projects/SUPERVIR/11-Results/Data_Treatment/3_created_data"
    assign("qPCR_path", set_microbs_created_DataPath(), envir = .GlobalEnv)
...
test_that("setting the custom local path for created data", {
    ...
    set_path <- set_microbs_created_DataPath(test_local_path)
    test_path <- get_microbs_created_DataPath()
